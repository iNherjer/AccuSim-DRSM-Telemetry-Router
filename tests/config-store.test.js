'use strict';

const assert = require('node:assert/strict');
const fs = require('node:fs');
const os = require('node:os');
const path = require('node:path');
const test = require('node:test');
const { BridgeConfigStore } = require('../lib/config-store');

test('config store creates and atomically persists bridge configuration', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'accusim-router-config-'));
  try {
    const store = new BridgeConfigStore({ dataDirectory: root });
    const initial = store.read();
    assert.equal(initial.port, 4135);
    assert.equal(initial.schemaVersion, 10);
    assert.equal(initial.language, 'de');
    assert.equal(initial.expertMode, false);
    assert.equal(initial.unsafeMode, false);
    assert.equal(initial.skippedUpdateVersion, '');
    assert.equal(initial.gravity.enabled, false);
    assert.equal(initial.gravity.strengthG, 1);
    assert.equal(initial.attitudeMix.enabled, false);
    assert.equal(initial.attitudeMix.pitchMix, 1);
    assert.equal(initial.attitudeMix.rollMix, 1);
    assert.equal(initial.groundForces.enabled, false);
    assert.equal(initial.groundForces.lateralMix, 1);
    assert.equal(initial.groundForces.longitudinalMix, 1);
    assert.equal(initial.groundForces.filterHz, 5);
    assert.equal(initial.groundForces.maxExtraG, 0.6);
    assert.equal(initial.groundForces.fadeInSeconds, 0.35);
    assert.equal(initial.groundForces.fadeOutSeconds, 1);
    assert.equal(initial.shakeMixer.enabled, false);
    assert.equal(initial.shakeMixer.strengthG, 0.08);
    assert.equal(initial.shakeMixer.centerHz, 0.8);
    assert.equal(initial.shakeMixer.smoothingHz, 12);
    assert.equal(initial.shakeMixer.maxExtraG, 0.15);
    assert.deepEqual(initial.shakeMixer.sources.airframe.mixes, [0.25, 0.25, 0.5]);
    assert.deepEqual(initial.shakeMixer.sources.vertical.mixes, [0, 0, 1]);
    assert.deepEqual(initial.shakeMixer.sources.horizontal.mixes, [1, 0, 0]);
    assert.equal(initial.rotationFusion.correctionEnabled, true);
    assert.equal(initial.rotationFusion.correctionTauSeconds, 1.25);
    assert.equal(initial.rotationFusion.residualWashoutEnabled, false);
    assert.equal(initial.rotationFusion.residualWashoutTauSeconds, 6);
    assert.equal(initial.rotationFusion.v2DetailMix, 0.55);
    assert.equal(initial.rotationFusion.v2CorrectionTauSeconds, 0.35);
    assert.equal(initial.rotationFusion.v2BiasTauSeconds, 2.5);
    assert.equal(initial.turbulence.enabled, false);
    assert.equal(fs.existsSync(path.join(root, 'bridge-config.json')), true);
    initial.host = '192.168.1.20';
    initial.channels.rpm_left.enabled = false;
    initial.skippedUpdateVersion = '1.3.1';
    initial.language = 'en';
    store.write(initial);
    const loaded = store.read();
    assert.equal(loaded.host, '192.168.1.20');
    assert.equal(loaded.channels.rpm_left.enabled, false);
    assert.equal(loaded.skippedUpdateVersion, '1.3.1');
    assert.equal(loaded.language, 'en');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});

test('invalid files safely fall back to defaults', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'accusim-router-invalid-'));
  try {
    fs.writeFileSync(path.join(root, 'bridge-config.json'), '{broken', 'utf8');
    const config = new BridgeConfigStore({ dataDirectory: root }).read();
    assert.equal(config.name, 'A2A_PA24_250_Comanche_MSFS');
    assert.equal(config.channels['acc.2'].enabled, true);
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
