...
class TelemetryRuntime extends EventEmitter {
  ...
  startRecording() {
    if (!this.recorder) return { ok: false, message: 'CSV-Ausgabeordner ist nicht konfiguriert.' };
    try {
  ...
  async connect(token) {
    ...
    try {
      ...
      this.setState({
        simulator: 'connected',
        detail: readableSources.length > 0
          ? `Verbunden: ${recvOpen.applicationName || 'Microsoft Flight Simulator'} · ${readableSources.length} aktive Quellen`
          : 'Verbunden · keine aktiven SimConnect-Quellen',
        lastError: ''
      });
    } catch (error) {
