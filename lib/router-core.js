...
const GROUND_ACCEL_COMPENSATION_CROSSOVER_HZ = 0.7;
const GROUND_LATERAL_DCS_SIGN = -1;
...
function requiredSources(config) {
  ...
  if (config?.groundForces?.enabled === true) {
    ...
    if (config.groundForces.accelerationCompensationEnabled === true) {
      selectedIds.add('std.velocity.world.x');
      selectedIds.add('std.velocity.world.y');
      selectedIds.add('std.velocity.world.z');
      selectedIds.add('std.heading');
    }
    if (config.groundForces.heaveStabilizationEnabled === true) {
      selectedIds.add('a2a.acc.y');
      selectedIds.add('std.gforce');
    }
  }
...
function normalizeConfig(rawConfig) {
  ...
  return {
    ...
    attitudeMix: {
      ...
      pitchMix: clamp(finite(rawAttitudeMix.pitchMix, defaults.attitudeMix.pitchMix), 0, 5),
      rollMix: clamp(finite(rawAttitudeMix.rollMix, defaults.attitudeMix.rollMix), 0, 5)
    },
    groundForces: {
      enabled: rawGroundForces.enabled === true,
      accelerationCompensationEnabled: rawGroundForces.accelerationCompensationEnabled === undefined
        ? defaults.groundForces.accelerationCompensationEnabled
        : rawGroundForces.accelerationCompensationEnabled === true,
      heaveStabilizationEnabled: rawGroundForces.heaveStabilizationEnabled === undefined
        ? defaults.groundForces.heaveStabilizationEnabled
        : rawGroundForces.heaveStabilizationEnabled === true,
...
class RouterCore {
  ...
  groundForcesSignal(sourceValues, timeSeconds) {
    ...
    const worldVelocityFps = [
      Number(sourceValues?.['std.velocity.world.x']),
      Number(sourceValues?.['std.velocity.world.z']),
      Number(sourceValues?.['std.velocity.world.y'])
    ];
    const headingDegrees = Number(sourceValues?.['std.heading']);
    const kinematicSourcesValid = worldVelocityFps.every(Number.isFinite) && Number.isFinite(headingDegrees);
    const a2aVerticalMps2 = Number(sourceValues?.['a2a.acc.y']);
    const standardGForce = Number(sourceValues?.['std.gforce']);
    const heaveSourcesValid = Number.isFinite(a2aVerticalMps2) && Number.isFinite(standardGForce);
    const previous = this.groundForceState;
    const dt = previous ? timeSeconds - previous.timeSeconds : 0;
    const validDt = dt > 0.001 && dt <= SAMPLE_GAP_THRESHOLD_SECONDS;
    let blend = previous && validDt ? previous.blend : 0;
    let heaveBlend = previous && validDt ? previous.heaveBlend : (onGround ? 1 : 0);
    let filteredG = previous && validDt ? [...previous.filteredG] : [...rawG];
    let standardSlowG = previous && validDt ? [...previous.standardSlowG] : [...rawG];
    let kinematicSlowG = previous && validDt ? [...previous.kinematicSlowG] : [0, 0];
    let kinematicG = [0, 0];
    let compensationValid = false;
    let compensatedG = previous && validDt ? [...previous.compensatedG] : [0, 0];

    if (previous && validDt && kinematicSourcesValid && previous.kinematicSourcesValid) {
      const worldAccelerationG = worldVelocityFps.map((value, index) => (
        (value - previous.worldVelocityFps[index]) * UNIT_DEFINITIONS.fps.factor / dt / 9.80665
      ));
      const heading = headingDegrees * UNIT_DEFINITIONS.degrees.factor;
      const sinHeading = Math.sin(heading);
      const cosHeading = Math.cos(heading);
      kinematicG = [
        worldAccelerationG[0] * cosHeading - worldAccelerationG[1] * sinHeading,
        worldAccelerationG[0] * sinHeading + worldAccelerationG[1] * cosHeading
      ];
      compensationValid = kinematicG.every(Number.isFinite);
    }
    ...
    if (previous && validDt) {
      ...
      heaveBlend += blendAlpha * ((onGround ? 1 : 0) - heaveBlend);
      ...
      if (onGround) {
        if (!previous.onGround) {
          ...
          filteredG = [...rawG];
          standardSlowG = [...rawG];
          kinematicSlowG = compensationValid ? [...kinematicG] : [0, 0];
          compensatedG = compensationValid ? [...kinematicG] : [...rawG];
        } else {
          ...
          if (config.accelerationCompensationEnabled && compensationValid) {
            const compensationAlpha = 1 - Math.exp(
              -2 * Math.PI * GROUND_ACCEL_COMPENSATION_CROSSOVER_HZ * dt
            );
            standardSlowG = previous.standardSlowG.map((value, index) => (
              value + compensationAlpha * (filteredG[index] - value)
            ));
            kinematicSlowG = previous.kinematicSlowG.map((value, index) => (
              value + compensationAlpha * (kinematicG[index] - value)
            ));
            compensatedG = filteredG.map((value, index) => (
              value - standardSlowG[index] + kinematicSlowG[index]
            ));
          } else {
            compensatedG = [...filteredG];
          }
        }
      }
    ...
    heaveBlend = clamp(heaveBlend, 0, 1);
    const mixes = [config.lateralMix, config.longitudinalMix];
    const forceG = config.accelerationCompensationEnabled && compensationValid
      ? compensatedG
      : filteredG;
    const unlimitedG = forceG.map((value, index) => (
      value * mixes[index] * (index === 0 ? GROUND_LATERAL_DCS_SIGN : 1)
    ));
    const maxExtraG = config.maxExtraG;
    const limitedG = unlimitedG.map((value) => maxExtraG * Math.tanh(value / maxExtraG));
    const groundHeaveG = heaveSourcesValid
      ? 1 + a2aVerticalMps2 * UNIT_DEFINITIONS.mps2.factor
      : 0;
    this.groundForceState = {
      timeSeconds,
      onGround,
      blend,
      heaveBlend,
    ...
    return {
      ...
      heave: {
        valid: heaveSourcesValid,
        blend: heaveBlend,
        groundG: groundHeaveG,
        flightG: heaveSourcesValid ? standardGForce : 0,
        rawDeltaG: heaveSourcesValid ? (groundHeaveG - standardGForce) * heaveBlend : 0
      }
    };
  }
    ...
    if (acceleration) {
      if (this.config.groundForces.enabled) {
        ...
        if (groundForces.error) {
          ...
          if (this.config.groundForces.heaveStabilizationEnabled && groundForces.heave.valid) {
            const channel = this.config.channels['acc.2'];
            const eligibleHeave = channel?.enabled === true &&
              channel.sourceId === 'std.gforce' &&
              channel.operation === 'direct';
            const heaveSign = channel?.invert === true ? -1 : 1;
            const appliedHeaveG = eligibleHeave
              ? groundForces.heave.rawDeltaG * finite(channel.scale, 1) * heaveSign
              : 0;
            if (eligibleHeave) {
              acceleration[2] = finite(acceleration[2], 0) + appliedHeaveG;
              outputValues['acc.2'] = acceleration[2];
            }
            diagnostics.groundForces.heave = {
              ...groundForces.heave,
              eligible: eligibleHeave,
              appliedG: appliedHeaveG
            };
          }
        }
      }
