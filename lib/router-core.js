...
class RouterCore {
  ...
  turbulenceSignal(sourceValues, timeSeconds) {
    ...
    const source = this.sourceMap.get(config.sourceId);
    const input = UNIT_DEFINITIONS[source?.inputUnit];
    const rawNumber = Number(sourceValues[config.sourceId]);
    if (!source || !input || !['acceleration', 'velocity'].includes(input.family)) {
      return { error: 'Turbulenzquelle muss Beschleunigung oder Geschwindigkeit liefern.' };
    }
    if (!Number.isFinite(rawNumber)) return { error: 'Turbulenzquelle liefert keine Zahl.' };

    const canonical = rawNumber * input.factor;
    const previous = this.turbulenceState;
    const dt = previous ? timeSeconds - previous.timeSeconds : 0;
    const validDt = dt > 0.001 && dt <= 0.25;
    ...
    const unlimitedExtraG = bandG * (config.gain - 1) * config.mix;
    const maxExtraG = config.maxExtraG;
    const extraG = maxExtraG * Math.tanh(unlimitedExtraG / maxExtraG);
    this.turbulenceState = { timeSeconds, canonical, slow, fast };
    return {
      raw: rawNumber,
      sourceG,
      bandG,
  ...
  update(sourceValues, timeSeconds) {
    ...
    const diagnostics = {
      gravity: { enabled: this.config.gravity.enabled, vectorG: [0, 0, 0] },
      turbulence: {
    ...
    if (acceleration) {
      if (this.config.gravity.enabled) {
        const vectorG = gravityVector(outputValues.pitch, outputValues.roll, this.config.gravity.strengthG);
        diagnostics.gravity.vectorG = vectorG;
      ...
      if (this.config.turbulence.enabled && this.config.channels['acc.2']?.enabled) {
        ...
        if (turbulence.error) {
          ...
          acceleration[2] = finite(acceleration[2], 0) + turbulence.extraG;
