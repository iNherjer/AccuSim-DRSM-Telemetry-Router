...
function requiredSources(config) {
  ...
  if (config?.gravity?.enabled === true) {
    ...
    // offset or mapped to another source.
...
function gravityReferenceAttitude(sourceValues) {
  ...
  return {
    // MSFS attitude signs are opposite to the DCS-v2 convention used here.
    pitch: valid ? -pitchDegrees * degreesToRadians : 0,
    roll: valid ? -rollDegrees * degreesToRadians : 0,
    valid
  };
}
