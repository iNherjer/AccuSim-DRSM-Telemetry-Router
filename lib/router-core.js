...
function requiredSources(config) {
  ...
  const usesAngularFusion = OUTPUTS.some((definition) => {
    const channel = safeObject(channels[definition.id]);
  ...
  if (config?.gravity?.enabled === true || config?.attitudeMix?.enabled === true || usesAngularFusion) {
  ...
  if (usesAngularFusion) selectedIds.add('std.heading');
...
class RouterCore {
  ...
  convert(definition, channel, rawValue, timeSeconds, angularReference = null) {
  ...
  update(sourceValues, timeSeconds) {
    ...
    const angularReference = usesAngularFusion
